...
const statusColors = {
  ...
  const openCreate = () => {
    ...
    try {
      await tablesApi.create({ number: form.number, capacity: Number(form.capacity), status: form.status })
      addToast('Table created','success')
      setModalOpen(false)
      load()
    } catch (err) { addToast(err.message ||'Failed to create','error') } } const handleUpdate = async () => {
    ...
    try {
      ...
      load()
    } catch (err) { addToast(err.message ||'Failed to update','error') } } const handleDelete = async (table) => {
    if (!confirm(`Delete table ${table.number}?`)) return
    try {
      await tablesApi.remove(table._id || table.id)
      addToast('Table deleted','success')
      load()
    } catch (err) { addToast(err.message ||'Failed to delete','error') } } return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-gray-400 => t.status ==='available').length} available of {tables.length} total</h2>
        {canWrite && (
      ...
      {loading ? <Spinner /> : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {tables.length === 0 ? (
            <div className="text-gray-400 tables yet. Add your first table to get started.</div>
          ) : tables.map((t) => (
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setModalOpen(false)} className="hover:bg-gray-100 rounded-lg">Cancel</button>
      ...
      <Modal isOpen={editModalOpen} onClose={() => { setEditModalOpen(false); setSelectedTable(null) }} title={`Table ${selectedTable?.number ||''}`}>
        <div className="space-y-4">
          <div className="text-gray-400 {selectedTable?.capacity} seats</div>
          <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm">
            <option value="available">Available</option>
            <option value="occupied">Occupied</option>
            <option value="reserved">Reserved</option>
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => { setEditModalOpen(false); setSelectedTable(null) }} className="hover:bg-gray-100 rounded-lg">Cancel</button>
