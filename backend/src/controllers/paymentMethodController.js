...
exports.create = async (req, res) => {
  const { name, description, isActive, icon, fields, provider, apiKey, publicKey, mode } = req.body
  if (!name) return res.status(400).json({ message: "Payment method name is required." })

  try {
    const id = await client.mutation(ref("paymentMethods:create"), { name, description, isActive: isActive !== false, icon, fields, provider, apiKey, publicKey, mode: mode || "sandbox" })
...
exports.update = async (req, res) => {
  ...
  try {
    const updated = await client.mutation(ref("paymentMethods:update"), { id: req.params.id, name, description, isActive: isActive !== false, icon, fields, provider, apiKey, publicKey, mode })
